...
impl Plane {
    fn new(id: u32, waited_time: u32) -> Plane {
        Plane { id, waited_time }
    }
}
...
impl Airport {
    ...
    fn request_runway(&self, plane_id: u32) -> u32 {
        let mut state = self.state.lock().unwrap();

        state.waiting_list.push_back(plane_id);
        
        while state.runways.iter().all(|r| r.is_occupied)
            || *state.waiting_list.front().unwrap() == plane_id
        {
...
fn main() {
    ...
    thread::scope(|s| {
        for plane in planes {
            ...
            s.spawn(move || {
                println!(
                    "Plane {}, requesting landing with priority {}",
                    plane_id, plane.waited_time
                );
                let runway_id = airport.request_runway(0);
